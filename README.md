...
- **Smart Decide (System One)**: MCP-based structured decision making via Gitee AI / Bespoke Nimble (`Bespoke-Nimble-9B`). Evaluates a `state` against typed questions (`noul` yes/no, `choice`, `score`) and returns one answer per question with calibrated probabilities — no free-form text generated. Exposes the `smart_decide` tool at `/smart-decide/mcp`. See [docs/modules/smart-decide.md](docs/modules/smart-decide.md).
