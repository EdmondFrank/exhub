    ...
    updated_chain =
      ...
      |> LLMChain.add_tools([
        Function.new!(%{
          name: "web_search",
          description:
            "Search the web using keywords, and return the titles, summaries, and links of the retrieved relevant content.",
          parameters_schema: %{
            type: "object",
            properties: %{
              keywords: %{
          ...
          function: fn args, _context ->
            ...
            with {result, 0} <-
                   System.cmd("python", ["-m", "webscout", "text", "-k", args["keywords"]],
                     env: [{"HTTPS_PROXY", Application.get_env(:exhub, :proxy, "")}]
                   ) do
              Logger.debug("After search with result: #{result}")
              {:ok, result}
            else
              _ -> {:ok, "failed to execute search tool with arguments: #{inspect(args)}"}
            end
          end
        }),
        Function.new!(%{
