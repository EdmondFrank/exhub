defmodule Exhub.Router.Config do
  ...
  Custom outbound headers (e.g. model/provider-specific package headers) can
  be configured at runtime in a JSON file without recompiling `config.exs` or
  restarting Exhub. The file is selected by `EXHUB_ROUTER_CONFIG` or defaults
  to `~/.config/exhub/router.json`. See `Exhub.Router.Settings` for the file
  format and `reload_router_settings/0` for explicit cache invalidation.
  `priv/router.json.example` shows the DeepSeek package-header configuration.

  When runtime custom headers apply to a forwarded request, only their header
  names are logged (at `info` level) so proxy forwarding remains observable
  without leaking header values.
  ...
  @doc """
  Returns the authorization header for a given model and provider type, merged
  with any custom outbound headers configured in the runtime router settings
  file (see `Exhub.Router.Settings`). Authentication headers are always
  generated by Exhub and cannot be overridden by JSON settings.
  ...
  # Logs the presence of runtime-configured custom headers so proxy forwarding
  # is observable. Only header names are emitted; values are never logged.
  defp log_custom_headers([], _model, _provider), do: :ok

  defp log_custom_headers(custom_headers, model, provider) do
    names = Enum.map(custom_headers, fn {name, _} -> name end)

    Logger.info(
      "[Router] Applying #{length(custom_headers)} custom header(s) for #{provider} model " <>
        "#{inspect(model)} on proxy forwarding: #{Enum.join(names, ", ")}"
    )
  end

  # Merges runtime custom headers over the base authentication headers.
  # Configured values take precedence for the same case-insensitive header
  # name. Credential headers supplied through the JSON settings are filtered
  # out by `Exhub.Router.Settings`, so authorization always stays under
  # Exhub's control.
  defp merge_custom_headers(base_headers, custom_headers) do
    custom_map =
      Enum.reduce(custom_headers, %{}, fn {name, value}, acc ->
        Map.put_new(acc, String.downcase(name), {name, value})
      end)

    replaced =
      Enum.map(base_headers, fn {name, value} ->
        case custom_map[String.downcase(name)] do
          nil -> {name, value}
          {custom_name, custom_value} -> {custom_name, custom_value}
        end
      end)

    base_names = MapSet.new(base_headers, fn {name, _} -> String.downcase(name) end)

    additions =
      Enum.reject(custom_headers, fn {name, _} ->
        MapSet.member?(base_names, String.downcase(name))
      end)
  ...
  @doc """
  Explicitly reloads the runtime router settings from disk.

  JSON settings are normally re-read automatically when the file changes, but
  this forces an immediate cache invalidation for operational tooling and
  tests.
  """
