defmodule Exhub.MCP.Tools.Agent.GrantPermission do
  ...
  schema do
    ...
    field(:option_id, {:required, :string},
      description: "The optionId to select from the permission options"
    )
  end
  ...
  def execute(%{agent_id: agent_id, session_id: session_id, option_id: option_id}, frame) do
    case Exhub.MCP.Agent.Store.resolve_permission(agent_id, session_id, option_id) do
      {:ok, _} ->
        resp =
          Response.tool()
          |> Response.text(Jason.encode!(%{status: "granted", option_id: option_id}))

        {:reply, resp, frame}

      {:error, reason} ->
        {:reply,
         Response.tool() |> Response.error("Failed to grant permission: #{inspect(reason)}"),
         frame}
    end
  end
end
