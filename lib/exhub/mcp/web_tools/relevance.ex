defmodule Exhub.MCP.WebTools.Relevance do
  @moduledoc """
  Precision filter for web search results, powered by the Smart Decide
  (System One) decision model.

  `Exhub.MCP.Tools.WebSearch` retrieves a pool of web page results from the
  Gitee AI web-search API and returns a fixed number of them regardless of
  whether they are actually relevant to the query. This module adds a second,
  ...
  Configuration lives under `:exhub, Exhub.MCP.WebTools.Relevance`; in-code
  ...
  Exhub.MCP.WebTools.Relevance` over the in-code defaults.
  """
  @spec config() :: keyword()
  def config do
  ...
  @doc """
  Filters `candidates` down to the results Smart Decide judges relevant to
  `query`.
    ...
    cond do
      ...
      true ->
        ...
        cond do
          ...
          fallback? ->
            Logger.debug(
              "[MCP WebTools] Relevance: nothing relevant for " <>
                "#{inspect(query)} — falling back to the search pool"
            )
  ...
  Keeps the result unless the `relevant` answer is a `noul` probability below
  ...
  defp collect(results) do
    {kept, errors} =
      Enum.reduce(results, {[], 0}, fn
        {page, :keep, nil}, {kept, errors} -> {[page | kept], errors}
        {page, :keep, _reason}, {kept, errors} -> {[page | kept], errors + 1}
        {_page, :drop, _reason}, {kept, errors} -> {kept, errors}
      end)

    if errors > 0 do
      Logger.debug("[MCP WebTools] Relevance: #{errors} judgment(s) failed (kept as relevant)")
    end
  ...
  defp judge_all(candidates, query, opts) do
    ...
    |> Enum.zip(candidates)
    |> Enum.map(fn
      {{:ok, result}, _page} -> result
      {{:exit, reason}, page} -> {page, :keep, {:timeout, reason}}
    end)
  end
  ...
  defp build_state(page, limit) do
    title = Map.get(page, "name") || Map.get(page, :name) || "unknown"
    url = Map.get(page, "url") || Map.get(page, :url) || ""
    snippet = Map.get(page, "snippet") || Map.get(page, :snippet) || ""
    summary = Map.get(page, "summary") || Map.get(page, :summary) || ""
  ...
  defp instructions(query, limit) do
    ...
    ~s(Decide whether the web search result described in the state could help with this task: "#{q}". ) <>
      "Answer yes if the result is relevant, related or plausibly useful — partial " <>
      "matches and sensible intermediate steps count. Answer no only if the result " <>
